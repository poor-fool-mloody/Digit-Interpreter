...
import csv
import operator
import matplotlib.pyplot as plt  # zaimportowanie biblioteki rysującej wykresy
...
def knn(train_data, test_data, k):
    ...
    output_file_errors2 = 'errors.csv'  # całkowita ilość popełnionych będów klasyfikacyjnych dla k=1..k

    error = [0 for x in range(
        k)]  # lista w której przechowywane są całkowite ilości popełnionych będów klasyfikacyjnych dla k=1..k sąsiadów
    ...
    with open(test_data, 'r') as test_data:  # otwarcie pliku z zestawem danych testujących
        ...
        with open(train_data, 'r') as train_data:  # otwarcie pliku z zestawem danych uczących
            ...
            with open(output_file_classification, 'w',
                      newline='') as output_file_classification:  # otwarcie pliku wyjściowego zawierającego zestaw danych sklasyfikowanych dla k=1..k
                fieldnames = ['test digit', 'digit clasified for k=1', 'k=2', 'k=3', 'k=4', 'k=5', 'k=6',
                              'k=7']  # ustalenie nazw kolumn w pliku classification.csv
                ...
                with open(output_file_errors, 'w',
                          newline='') as output_file_errors:  # otwarcie pliku wyjściowego zawierającego całkowitą iloś popełnionych będów klasyfikacyjnych dla k=1..k sąsiadów
                    fieldnames2 = ['k', 'number of errors']  # ustalenie nazw kolumn w pliku errors.csv
                    ...
                    for test_digit in test_reader:  # pętla sczytująca każdą cyfrę testującą z zestawu cyfr testujących
                        ...
                        train_data.seek(
                            0)  # ustawienie wskaźnika aktualnego czytania danych uczącyhc z pliku na początek
                        ...
                        for train_digit in train_reader:  # pętla sczytująca każdą cyfrę uczącą z zestawu cyfr uczących
                            distance = euclidean_distance(train_digit,
                                                          test_digit)  # lista przechowująca wartość cyfry testującej oraz jej wartość odległości pomiędzy wektorem tej cyfry od wektora cyfry uczącej
                    ...
                    for x in range(k):  # wypisywanie błędów klasyfikacyjnych dla k=1..k sąsiadów do pliku errors.csv
                        output_writer2.writerow({fieldnames2[0]: x + 1, fieldnames2[1]: error[x]})

    with open(output_file_errors2, 'r') as output_file_errors:  # otwarcie pliku z błędami klasyfikacji
        ...
        plt.show()  # funkcja rysująca wykres
...
def classify(distances, k):  # funkcja klasyfikująca cyfrę
    ...
    distances.sort(key=operator.itemgetter(
        1))  # sortowanie (od najmniejszej wartości) listy przechowującej odległości pomiędzy cyfrą testującą a uczącą
    ...
    for x in range(k):  # pętla iterująca od 1 do k odległości w liście distances
        ...
        sorted_classification_digits_table.append(sorted_classification_digits[0][
                                                      0])  # weź pod uwagę cyfrę sklasyfikowaną która ma największą ilość wystąpień w słowniku
...
def errors(classified,
           test_digit,
           error):  # funkcja zwiększająca licznik błędów dla k=1..k sąsiadów jeśli sklasyfikowana cyfra różni się od rzeczywistej cyfry
...
def main():
    ...
    knn(train_data, test_data,
        k)  # knn - funkcja klasyfikująca algorytmem k najbliższych sąsiadów zadane cyfry arabskie z zestawu testujących danych MNIST na podstawie uczącego zestawu danych MNIST.
